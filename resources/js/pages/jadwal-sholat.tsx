    ...
    useEffect(() => {
        const updateClock = () => {
            ...
            if (Object.keys(jadwal).length > 0) {
                ...
                if (nextTimeStr) {
                    const [nh, nm] = nextTimeStr.split(" ")[0].split(":").map(Number);
                    ...
                    const countdownEl = document.querySelector(".next-prayer-countdown");
                    if (countdownEl) {
                        countdownEl.textContent = `-${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}:${String(s).padStart(2, "0")}`;
                    }

                    const nextLabelEl = document.querySelector(".next-prayer-label");
                    if (nextLabelEl) {
                        const labels: any = { subuh: "Subuh", terbit: "Terbit", dhuha: "Dhuha", dzuhur: "Dzuhur", ashar: "Ashar", maghrib: "Maghrib", isya: "Isya", imsak: "Imsak" };
    ...
    return (
        <>
            ...
            breadcrumbs={[
                ...
                { label: 'Layanan' },
                { label: "Jadwal Sholat", icon: <Clock className="h-4 w-4" /> },
            ]}
            />
            ...
            <div className="mx-auto max-w-380 px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col gap-8 lg:col-span-4 sticky top-24">
                    {/* Jadwal Besok */}
                    {!loading && Object.keys(jadwalBesok).length > 0 && (
                    <TomorrowSchedule
