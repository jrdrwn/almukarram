...
export default function OpiniDetailPage({
    ...
    const seoDescription =
        opini.excerpt?.trim() ||
        opini.isi.replace(/<[^>]+>/g, '').slice(0, 160);
    const opiniPath = `/opini-detail/${opini.slug}`;

    return (
        <>
            <Seo
                title={`${opini.title} | Opini — Masjid Agung Al-Mukarram`}
                description={seoDescription}
                image={opini.foto || undefined}
                url={opiniPath}
                type="article"
                author={opini.author}
            />
            ...
            <div className="fixed right-6 bottom-6 z-50">
                <div className="flex flex-col gap-1.5 overflow-hidden rounded-2xl border border-border bg-white/80 p-2 shadow-lg backdrop-blur-xl dark:bg-zinc-900/80">
                    ...
                    <button
                        ...
                        className="rounded-xl bg-violet-50 px-3 py-1.5 text-sm font-extrabold text-violet-700 transition hover:bg-violet-100 active:bg-violet-100 dark:bg-violet-900/20 dark:text-violet-400 dark:hover:bg-violet-900/40 dark:active:bg-violet-900/40"
                    ...
                    <button
                        ...
                        className="rounded-xl bg-zinc-100 px-3 py-1.5 text-sm font-bold text-zinc-600 transition hover:bg-zinc-200 active:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 dark:active:bg-zinc-700"
