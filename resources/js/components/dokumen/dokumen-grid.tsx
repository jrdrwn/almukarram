        ...
        <div className="flex flex-col gap-8">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                {data.map((doc, idx) => (
                    <div
                        key={doc.id}
                        className="group relative flex h-full flex-col overflow-hidden rounded-4xl border border-border/50 bg-white/60 p-6 backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-emerald-500/5 dark:bg-zinc-900/60"
                        style={{ animationDelay: `${idx * 100}ms` }}
                    >
                        {/* Background Decoration */}
                        <div className="pointer-events-none absolute top-0 right-0 h-32 w-32 rounded-bl-full bg-emerald-500/5 transition-transform duration-500 group-hover:scale-110" />

                        <div className="relative z-10 flex flex-1 items-start gap-4">
                            <div
                                className={cn(
                                    'flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl transition-transform duration-300 group-hover:-translate-y-1 group-hover:shadow-md',
                                    doc.bg,
                                    doc.color,
                                )}
                            >
                            ...
                            <div className="min-w-0 flex-1 space-y-4">
                                <h3 className="line-clamp-3 text-base leading-snug font-bold text-foreground transition-colors group-hover:text-emerald-600">
                                    {doc.title}
                                </h3>
                        ...
                        <div className="relative z-10 mt-auto pt-6">
                            <a
                                href={doc.url}
                                download={doc.url !== '#'}
                                className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-50 px-5 py-2.5 text-sm font-semibold text-emerald-600 transition-all duration-300 hover:bg-emerald-600 hover:text-white sm:w-auto dark:bg-emerald-500/10 dark:text-emerald-400"
                            >
