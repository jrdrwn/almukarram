...
export default function WeeklySchedule({
    ...
    return (
        <div className="group relative overflow-hidden rounded-4xl border border-gray-100 bg-white px-6 py-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] sm:p-10">
            ...
            <div className="absolute -right-20 -top-20 z-0 h-75 w-75 rounded-full bg-emerald-50 blur-[80px] transition-transform duration-700 group-hover:scale-110"></div>
            ...
            <div className="relative z-10 overflow-x-auto scrollbar-hide">
                <div className="min-w-150 pb-4">
                    <div className="grid grid-cols-6 gap-2 px-2 pb-4 text-[0.7rem] font-bold uppercase tracking-[0.15em] text-gray-400">
                    ...
                    <div className="flex flex-col gap-2.5">
                        {Object.entries(jadwal7Hari).map(([tgl, j]: any) => {
                            ...
                            const dayNameEn = dateObj.toLocaleDateString("id-ID", { weekday: "long" });
                            const dayName = hariId[dateObj.toLocaleDateString("en-US", { weekday: "long" })] || dayNameEn;
                            ...
                            return (
                                ...
                                >
                                    <div className="col-span-1 flex items-center gap-4 pl-2">
                                        ...
                                        <div>
                                            <div className={`font-bold tracking-tight ${isToday ? "text-white" : "text-gray-900"}`}>
                                                {dayName}
                                            </div>
                                    ...
                                    <div className={`text-center font-mono text-base font-bold ${!isToday && "text-gray-700"}`}>{j.subuh || "—"}</div>
                                    <div className={`text-center font-mono text-base font-bold ${!isToday && "text-gray-700"}`}>{j.dzuhur || "—"}</div>
                                    <div className={`text-center font-mono text-base font-bold ${!isToday && "text-gray-700"}`}>{j.ashar || "—"}</div>
                                    <div className={`text-center font-mono text-base font-bold ${!isToday && "text-gray-700"}`}>{j.maghrib || "—"}</div>
                                    <div className={`text-center font-mono text-base font-bold ${!isToday && "text-gray-700"}`}>{j.isya || "—"}</div>
                                </div>
                            );
                        })}
                    </div>
