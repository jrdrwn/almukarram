...
export default function BuletinSearch({ searchQuery = '', kategoriSlug = 'semua', totalRows = 0, kategoris = [] }: BuletinSearchProps) {
    ...
    const submitSearch = (payload: any) => {
        ...
        router.get('/buletin', params as any, { preserveState: true, preserveScroll: true });
    };
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    >
                        ...
                        <SelectContent className="rounded-xl border-zinc-200 dark:border-zinc-800 shadow-lg">
                            <SelectItem value="semua" className="cursor-pointer">Semua Tahun</SelectItem>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2 text-sm z-0">
                    <span className="flex items-center flex-wrap gap-1.5 bg-emerald-50 dark:bg-emerald-900/20 px-4 py-2 rounded-xl border border-emerald-100 dark:border-emerald-800/30 text-muted-foreground shadow-sm">
                        ...
                        {data.kategori && data.kategori !== 'semua' && (
                            <>
                                Tahun: <strong className="text-emerald-700 dark:text-emerald-400 font-semibold max-w-37.5 truncate">{kategoris.find(k => k.slug === data.kategori)?.nama || data.kategori}</strong>
                                <span className="mx-1 text-zinc-300 dark:text-zinc-700">•</span>
                            </>
                        )}
                        {(data.from || data.to) && (
                            <>
                                Tanggal: <strong className="text-emerald-700 dark:text-emerald-400 font-semibold">
                                    {data.from ? format(new Date(data.from), "d MMM yyyy", { locale: localeId }) : 'Awal'}
                                    {' - '}
                                    {data.to ? format(new Date(data.to), "d MMM yyyy", { locale: localeId }) : 'Sekarang'}
                                </strong>
                                <span className="mx-1 text-zinc-300 dark:text-zinc-700">•</span>
                            </>
                        )}
                        ...
                        Ditemukan <strong className="text-emerald-700 dark:text-emerald-400">{totalRows}</strong> buletin.
                    </span>
                    <Button
                        variant="ghost"
                        size="sm"
